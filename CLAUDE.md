...
**Destroy safety.** Destructive operations go through a single-pass interactive gate before the unified orchestrator will execute a destroy: enumerate the real resources from live Terraform state, print them, run the read-only pre-destroy guards, write the durable evidence record (`scripts/lib/destroy-evidence.py`, stdlib-only Python invoked via CLI, never imported as a package), then drain any buffered terminal input and require the operator to type exactly `yes` on `/dev/tty`. Draining is load-bearing — it is what stops a pasted `<command>\nyes` block from answering a prompt the human never saw. No tty means fail closed, and `--auto-approve` does not skip the gate in any environment. `record_pre_destroy_guard_result` in `orchestrator.sh` is the sole channel a guard uses to report results. Never bypass this by calling Terraform/kubectl destroy commands directly for anything beyond your own personal dev sandbox.
