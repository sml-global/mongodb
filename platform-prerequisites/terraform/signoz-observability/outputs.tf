...
output "alert_ids" {
  ...
  value = {
    mongodb_no_data                = signoz_rule.mongodb_no_data.id
    postgres_cpu_high              = signoz_rule.postgres_cpu_high.id
    k8s_node_cpu_high              = signoz_rule.k8s_node_cpu_high.id
    otel_collector_export_failures = signoz_rule.otel_collector_export_failures.id
    app_telemetry_no_data          = signoz_rule.app_telemetry_no_data.id
  }
}
