check "backup_bucket_name_is_provided" {
  assert {
    condition     = length(var.backup_bucket_name) > 0
    error_message = "backup_bucket_name must be provided; this root's cnpg-prereqs module creates the bucket using this name."
  }
}
