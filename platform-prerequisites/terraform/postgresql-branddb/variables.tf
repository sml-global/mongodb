...
variable "namespace" {
  description = "Namespace the brand CNPG cluster runs in (passed through to the cnpg-prereqs module)."
  type        = string
  default     = "branddb"
}

variable "workload_service_account_name" {
  description = "ServiceAccount used by the brand CNPG workload pods (passed through to the cnpg-prereqs module)."
  type        = string
  default     = "oms-postgresql-brand-workload"
}

variable "backup_bucket_name" {
  description = "S3 bucket name used by barman-cloud for the brand cluster's WAL archival and backups. Created by the cnpg-prereqs module using this name."
  type        = string
}

variable "iam_role_name" {
  description = "IAM role name for the brand CNPG workload's backup/encryption access (passed through to the cnpg-prereqs module)."
  type        = string
  default     = "postgresql-branddb-cnpg-role"
}
