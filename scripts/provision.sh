...
if [[ "${1:-}" != "--env" ]]; then
  if [[ "${1:-}" != "-h" && "${1:-}" != "--help" ]]; then
    ...
    echo "This will only ever provision the 'dev' environment. Use 'scripts/provision.sh --env uat <scope>' for UAT or 'scripts/provision.sh --env prod <scope>' for Production." >&2
