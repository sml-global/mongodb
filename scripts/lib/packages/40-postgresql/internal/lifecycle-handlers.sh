...
postgresql_internal_destroy_postgresql_core() {
  ...
  if [[ "${ENVIRONMENT:-}" != "dev" ]]; then
    postgresql_internal_error "postgresql-core destroy is not yet environment-aware (issue #95) — it would target the DEV-hardcoded legacy script regardless of \$ENVIRONMENT=${ENVIRONMENT:-<unset>}. Refusing to run against a non-dev environment."
    return 1
  fi
...
postgresql_internal_destroy_postgresql_brand() {
  ...
  if [[ "${ENVIRONMENT:-}" != "dev" ]]; then
    postgresql_internal_error "postgresql-brand destroy is not yet environment-aware (issue #95) — it would target the DEV-hardcoded legacy script regardless of \$ENVIRONMENT=${ENVIRONMENT:-<unset>}. Refusing to run against a non-dev environment."
    return 1
  fi
